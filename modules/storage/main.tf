...
resource "aws_security_group" "snyk_rds_sg2" {
  name   = "snyk_rds_sg2"
  vpc_id = var.vpc_id

  tags = merge(var.default_tags, {
    Name = "snyk_rds_sg2_${var.environment}"
  })
...
resource "aws_s3_bucket" "snyk_storage" {
  bucket = "snyk-storage-${var.environment}-demo-${random_id.example.hex}"
  tags = merge(var.default_tags, {  
...
resource "aws_s3_bucket" "my-new-undeployed-bucket" {
  bucket = "snyk-public-${var.environment}-demo-${random_id.example.hex}"
}
